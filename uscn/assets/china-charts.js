/* China profile charts */
(function () {
  var style = getComputedStyle(document.documentElement);
  var accent = style.getPropertyValue('--accent').trim();     // 中国红
  var accent2 = style.getPropertyValue('--accent2').trim();   // 基准蓝
  ...
  gdp.setOption({
    ...
    series:[
      { name:'中国', type:'line', smooth:true, symbolSize:7, data:[0.36,1.21,6.09,14.69,19.50], itemStyle:{color:accent}, lineStyle:{width:3,color:accent}, areaStyle:{color:accent+'22'} },
      { name:'美国', type:'line', smooth:true, symbolSize:7, data:[5.96,10.25,15.05,21.35,30.77], itemStyle:{color:accent2}, lineStyle:{width:3,color:accent2}, areaStyle:{color:accent2+'1f'} }
    ]
  });
  window.addEventListener('resize', function(){ gdp.resize(); });

  /* --- chart-energy: China renewable capacity --- */
  ...
  en.setOption({
    ...
    xAxis:{ type:'value', name:'亿千瓦', nameTextStyle:{color:muted}, axisLabel:{color:muted}, splitLine:{lineStyle:{color:rule}} },
    yAxis:{ type:'category', data:['太阳能','风电','水电与其他'], axisLine:{lineStyle:{color:rule}}, axisLabel:{color:ink} },
    series:[{
      name:'2025 装机', type:'bar', barWidth:22,
      data:[{value:12,itemStyle:{color:accent}},{value:6.4,itemStyle:{color:accent}},{value:5.0,itemStyle:{color:accent+'aa'}}],
  ...
  co2.setOption({
    ...
    series:[
      { name:'中国', type:'bar', barWidth:64, data:[155.4], itemStyle:{color:accent}, label:{show:true,position:'top',formatter:'155.4亿吨\n占全球29.2%',color:muted,fontSize:11} },
      { name:'美国', type:'bar', barWidth:64, data:[59.1], itemStyle:{color:accent2}, label:{show:true,position:'top',formatter:'59.1亿吨\n占全球11.1%',color:muted,fontSize:11} }
    ]
  });
  ...
  radar.setOption({
    ...
    series:[{
      ...
      data:[
        { name:'中国', value:[8.0,7.2,7.4,9.6,9.6,8.2,7.6,6.9,7.4,9.4], itemStyle:{color:accent}, areaStyle:{color:accent+'2e'} },
        { name:'美国', value:[9.2,9.7,9.9,8.2,7.2,8.8,9.4,9.2,9.0,7.0], itemStyle:{color:accent2}, areaStyle:{color:accent2+'24'} }
      ]
    }]
  });
